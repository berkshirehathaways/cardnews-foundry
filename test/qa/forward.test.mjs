...
test("Given the fresh-create selector, When forward contexts are selected, Then exactly one fresh Codex workflow runs", async () => {
  // Given
  const { selectForwardContextSpecs } = await import("../../scripts/qa-forward.mjs");
  const specs = [
    { scenario: "fresh-create" },
...
test("Given no forward context selector, When forward contexts are selected, Then all existing contexts remain enabled", async () => {
  // Given
  const { selectForwardContextSpecs } = await import("../../scripts/qa-forward.mjs");
...
test("Given the skip selector, When forward contexts are selected, Then no fresh Codex workflow runs", async () => {
  // Given
  const { selectForwardContextSpecs } = await import("../../scripts/qa-forward.mjs");
  const specs = [{ scenario: "fresh-create" }, { scenario: "interrupted-resume" }];
...
test("Given a fully accepted fresh-create job with a completed package, When its F4 filesystem outcome is assessed, Then the seven-card evaluation passes", async () => {
  // Given
  const { freshCreateOutcomePassed } = await import("../../scripts/qa-forward.mjs");
  const outcome = {
    candidateCount: 1,
    candidates: [{
      checks: {
...
test("Given a fresh-create job without an accepted evaluation, When its F4 filesystem outcome is assessed, Then it fails", async () => {
  // Given
  const { freshCreateOutcomePassed } = await import("../../scripts/qa-forward.mjs");
  const outcome = {
    candidateCount: 1,
...
test("Given one recovered accepted job beside an incomplete fresh attempt, When its F4 filesystem outcome is assessed, Then the completed job passes", async () => {
  // Given
  const { freshCreateOutcomePassed } = await import("../../scripts/qa-forward.mjs");
  const outcome = {
    candidateCount: 2,
...
test("Given two fully accepted fresh-create jobs, When their F4 filesystem outcome is assessed, Then the contaminated workspace fails", async () => {
  // Given
  const { freshCreateOutcomePassed } = await import("../../scripts/qa-forward.mjs");
  const checks = {
    statusExit: true,
  ...
  const outcome = { candidateCount: 2, candidates: [{ checks }, { checks }] };
