...
import { access, chmod, cp, mkdir, mkdtemp, readFile, readdir, rm, rmdir, writeFile } from "node:fs/promises";
...
test("Given accepted asset files are replaced, When projection or revision reads them, Then digest and metadata integrity fail closed", async (context) => {
  const workspace = await mkdtemp(path.join(os.tmpdir(), "cardnews-cli-asset-integrity-"));
  context.after(() => rm(workspace, { recursive: true, force: true }));
  const source = path.join(workspace, "article.html");
  const assetA = path.join(workspace, "seed-orbit.bin");
  const assetB = path.join(workspace, "record-grid.bin");
  ...
  await cp(path.join(synthetic, "assets", "record-grid", "asset.bin"), assetB);
  const initialized = await runJson(workspace, [
    "init", "--slug", "asset-integrity", "--target", "portrait-social-1080x1350", "--cards", "7"
  ]);
  const job = initialized.output.result.jobPath;
  const ingested = await runJson(workspace, [
    "ingest", "--job", job, "--file", source, "--allowed-root", workspace
  ...
  const artifactPath = path.join(workspace, importedA.output.result.artifactPath);
  const metadataPath = path.join(workspace, importedA.output.result.metadataPath);
  const originalAsset = await readFile(artifactPath);
  const originalMetadata = await readFile(metadataPath);

  await chmod(artifactPath, 0o600);
  await writeFile(artifactPath, Buffer.from(originalAsset).fill(0, 0, 1));
  const projectionRejection = await runJson(workspace, ["render", "--job", job], 3);
  const scaffold = await runJson(workspace, [
    "scaffold-record", "--job", job, "--stage", "editorial-brief"
  ]);
  const revisedEditorial = await readJson(path.join(synthetic, "records", "editorial-brief.json"));
  revisedEditorial.sourceEnvelopeDigest = ingested.output.result.contractDigest;
  revisedEditorial.thesis = `${revisedEditorial.thesis} Revised.`;
  await writeFile(
    path.join(workspace, scaffold.output.result.draftPath),
    `${canonicalJson(revisedEditorial)}\n`
  );
  const revisionRejection = await runJson(workspace, [
